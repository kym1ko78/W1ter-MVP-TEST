...
- Composer в окне чата переработан под более компактную message bar: кнопка `Очистить` убрана, attach action перенесен в иконку-скрепку, а send button появляется только при наличии текста или выбранного файла
- Из composer убрана текстовая подсказка про `Enter / Ctrl+Enter`; белая панель ввода расширена и плотнее обволакивает кнопку-скрепку слева
- Composer дополнительно ужат по высоте: текстовая строка, скрепка и появляющаяся кнопка отправки теперь выровнены по одной центральной оси
- После успешной отправки сообщения чат теперь автоматически прокручивается вниз к последнему сообщению, не затрагивая обычную ручную прокрутку истории
- Короткие text-only bubbles дополнительно ужаты по внутренним отступам и расстоянию между текстом и временем, чтобы сообщения выглядели компактнее без уменьшения шрифта
- Placeholder и вводимый текст в composer дополнительно выровнены по вертикальному центру поля ввода, чтобы строка не выглядела прижатой к верхнему краю
- Первый auth-экран упрощен: слева оставлен только логотип `W1ter`, приветственная подпись над правым заголовком убрана, а бейдж `live` заменен на `Alfa`
- Темная auth-панель теперь центрируется в своей колонке, а оставшийся белый фон экрана оформлен мягкими вертикальными полосами
- Auth-layout дополнительно перестроен: темная панель больше не привязана к правой колонке и теперь центрируется относительно всего экрана, а логотип вынесен в отдельный overlay-слой
- Полосатый фон auth-сцены усилен: белое полотно теперь собирается из более заметного многослойного вертикального паттерна, но остается вторичным по отношению к центральной панели входа
- Ровные полосы на белом auth-фоне заменены на темный органичный паттерн из хаотичных округлых вертикальных форм, чтобы фон выглядел живее и выразительнее
- Полосатый мотив на auth-фоне затем полностью убран: белая сцена теперь оформлена более чисто через крупные мягкие темные акценты по краям без ритмичных линий
- В composer строка ввода сообщения дополнительно центрирована относительно скрепки: счетчик вынесен из обычного потока, поэтому placeholder и вводимый текст больше не тянутся вверх
- Переключение между микрофоном и отправкой в composer теперь собрано в один action-slot: send-кнопка больше не приезжает сбоку, а мягко появляется из позиции mic через overlap-анимацию
- Overlap-артефакт между иконками mic/send устранен: переход в action-slot теперь разведен по фазам, поэтому glyph-слои больше не смешиваются визуально
- Composer action-slot переработан до одной реальной кнопки с внутренней сменой состояния mic/send, поэтому send-иконка теперь исчезает полностью и больше не просвечивает за микрофоном
- Страница `/verify-email` больше не ломает production build: client-часть с `useSearchParams()` обернута в `Suspense`, и `pnpm verify:web` снова проходит полностью
- Tracked build-артефакты `apps/web/.next-e2e` и `*.tsbuildinfo` удалены из Git index; `.gitignore` теперь действительно удерживает их вне коммитов, а для `apps/web/next-env.d.ts` добавлена стабильная LF-политика через `.gitattributes`
- `scripts/run-web-e2e-build.mjs` теперь восстанавливает исходный `apps/web/next-env.d.ts` после e2e-сборки, поэтому `pnpm test:ui:e2e:auto` больше не переключает route-types reference с `.next` на `.next-e2e`
